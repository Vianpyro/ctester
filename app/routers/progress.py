...
NOTHING IS COMPUTED BY THE BROWSER. Everything arrives ready-made from this
route, which recomputes the projection on every call from three append-only
fact tables and the public catalog. A page that computed its own XP would be
a page where one gives it to oneself from the console.
...
ponytail: six serialized SQL round trips behind `state.py`'s single lock.
