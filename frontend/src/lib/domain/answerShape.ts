...
export function shapeNote(type: string, given: string): string {
  ...
  switch (type) {
    case "bin8": {
      const s = bits(text);
      if (!s || !/^[01]+$/.test(s)) return "des 0 et des 1 seulement";
      // The judge says the same thing once it has the value: "l'énoncé demande 8 bits".
      return s.length === 8 ? "" : `${s.length} bit${s.length > 1 ? "s" : ""} sur 8`;
    }
    case "bin": {
      const s = bits(text);
      return !s || !/^[01]+$/.test(s) ? "des 0 et des 1 seulement" : "";
    }
    case "hex8": {
      ...
      return !s || !/^[0-9a-f]+$/.test(s) ? "des chiffres de 0 à 9 et de A à F" : "";
    }
    case "int": {
      const s = bare(text).replace("−", "-").replace(/^[-+]/, "");
      return !s || !/^[0-9]+$/.test(s) ? "un entier, sans autre caractère" : "";
    }
    case "number":
      return NUMBER.test(bare(text).replace("−", "-")) ? "" : "un seul nombre";
    default:
      return "";
  }
}
